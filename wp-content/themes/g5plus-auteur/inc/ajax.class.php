...
if ( ! class_exists( 'G5Plus_Auteur_Ajax' ) ) {
	class G5Plus_Auteur_Ajax {
		...
		public function search_result() {
			...
			$search_popup_post_type     = G5Plus_Auteur()->options()->get_search_popup_post_type();
			$search_popup_result_amount = G5Plus_Auteur()->options()->get_search_popup_result_amount();
            ...
            <ul class="search-popup-list">
				<?php if ( $query->have_posts() ): ?>
					...
					while ( $query->have_posts() ) {
						$query->the_post();
						G5Plus_Auteur()->helper()->getTemplate( 'popup/content' );
						wp_reset_postdata();
					}
					?>
				<?php else: ?>
                    <li class="sa-nothing"><?php esc_html_e( 'Sorry, but nothing matched your search terms. Please try again with different keywords.', 'g5plus-auteur' ); ?></li>
				<?php endif; ?>
            </ul>
			<?php
			echo ob_get_clean();
		...
		public function pagination_ajax_response() {
			...
			$query_args = G5Plus_Auteur()->query()->parse_ajax_query( $query_args );
		...
		public function gsf_user_sign_up_ajax_callback() {
			...
			if ( $error['errors']->get_error_code() != '' ) {
				...
				foreach ( $error['errors']->get_error_messages() as $key => $value ) {
					$message .= '<div/><strong>' . esc_html__( 'Error:', 'g5plus-auteur' ) . '</strong> ' . esc_html( $value ) . '</div>';
				}
			} else {
		...
		public function popup_product_quick_view() {
			...
			G5Plus_Auteur()->helper()->getTemplate( 'woocommerce/loop/content-product-quick-view' );
			wp_reset_postdata();
			die();
		}
		...
		public function portfolio_gallery() {
			...
			$media_type   = G5Plus_Auteur()->metaBoxPortfolio()->get_single_portfolio_media_type( $portfolio_id );
			$items        = array();

			if ( $media_type === 'image' ) {
				$portfolio_gallery = G5Plus_Auteur()->metaBoxPortfolio()->get_single_portfolio_gallery( $portfolio_id );
				...
				$portfolio_video = G5Plus_Auteur()->metaBoxPortfolio()->get_single_portfolio_video( $portfolio_id );
